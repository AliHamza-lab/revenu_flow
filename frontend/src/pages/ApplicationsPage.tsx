            ...
            <main className="flex-1 p-10 overflow-y-auto">
                <header className="flex justify-between items-end mb-12">
                    <div className="text-left">
                        ...
                        <h1 className="text-5xl font-black italic uppercase tracking-tighter">Tactical Applications</h1>
                    </div>
                    <button
                        onClick={() => setShowAdd(!showAdd)}
                        className="px-8 py-4 bg-red-600 rounded-2xl flex items-center gap-3 font-black uppercase tracking-widest text-xs hover:bg-red-700 transition-all shadow-[0_0_30px_rgba(255,0,0,0.2)]"
                    >
                ...
                {showAdd && (
                    <div className="mb-12 p-8 bg-white/[0.02] border border-white/5 rounded-[40px] animate-in fade-in slide-in-from-top-4">
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
                            <div className="flex flex-col gap-2">
                                ...
                                <input
                                    ...
                                    className="bg-black border border-white/10 rounded-2xl py-4 px-6 text-xs font-bold tracking-widest focus:outline-none focus:border-red-500 transition-all uppercase"
                                />
                            </div>
                            <div className="flex flex-col gap-2">
                                ...
                                <input
                                    ...
                                    required
                                    placeholder="EX: STAFF ARCHITECT"
                                    className="bg-black border border-white/10 rounded-2xl py-4 px-6 text-xs font-bold tracking-widest focus:outline-none focus:border-red-500 transition-all uppercase"
                                />
                            </div>
                            <div className="flex flex-col gap-2">
                                ...
                                <select
                                    ...
                                    className="bg-black border border-white/10 rounded-2xl py-4 px-6 text-xs font-bold tracking-widest focus:outline-none focus:border-red-500 transition-all uppercase appearance-none"
                                >
                            ...
                            <button
                                type="submit"
                                className="py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-neutral-200 transition-all"
                            >
                                EXECUTE LOGGING
                            </button>
                        </form>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-white/5 text-[10px] font-black text-gray-500 uppercase tracking-[0.3em]">
                                ...
                                <th className="p-8">Primary Objective</th>
                                <th className="p-8">Operational Status</th>
                                <th className="p-8">Commencement</th>
                            </tr>
                        </thead>
                        <tbody className="text-sm font-medium">
                            {loading ? (
                                ...
                                <tr key={app.id} className="border-b border-white/5 hover:bg-white/[0.03] transition-all group">
                                    ...
                                    <td className="p-8 opacity-60 uppercase tracking-widest text-xs font-bold">{app.job_title}</td>
                                    <td className="p-8">
                                        <span className={`px-4 py-1.5 rounded-full text-[9px] font-black tracking-[0.2em] border uppercase ${getStatusColor(app.status)}`}>
                                            {app.status}
                                        </span>
                                    </td>
                                    <td className="p-8 opacity-40 uppercase tracking-widest text-[10px] font-bold">
